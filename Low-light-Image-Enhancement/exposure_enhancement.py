...
def create_spacial_affinity_kernel(spatial_sigma: float, size: int = 15):
    # creates a kernel used for computing spatial affinity based Gaussian weights.
...
def compute_smoothness_weights(L: np.ndarray, x: int, kernel: np.ndarray, eps: float = 1e-3):
    # computing the smoothness weights for the illumination map.
...
def fuse_multi_exposure_images(im: np.ndarray, under_ex: np.ndarray, over_ex: np.ndarray,bc: float = 1, bs: float = 1, be: float = 1):
    # fuses under-exposure and over-exposure corrected images to produce a well-exposed image.
...
def refine_illumination_map_linear(L: np.ndarray, gamma: float, lambda_: float, kernel: np.ndarray, eps: float = 1e-3):
    # Defines the illumination map based on an optimization problem described in the DUAL and LIME papers
...
def enhance_image_exposure(im: np.ndarray, gamma: float, lambda_: float, dual: bool = True, sigma: int = 3,bc: float = 1, bs: float = 1, be: float = 1, eps: float = 1e-3):
    #Enhance input image, using either DUAL method, or LIME method
