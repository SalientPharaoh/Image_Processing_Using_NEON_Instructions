...
def init(img, _blk_size, _Beta_Kaiser):
    # initialization of the noisy image matrix
    m_shape = img.shape
...
def Locate_blk(i, j, blk_step, block_Size, width, height):
    # find the top left corner of the block based on current indices nad block size
...
def Define_SearchWindow(_noisyImg, _BlockPoint, _WindowSize, Blk_Size):
    # defines the search window for block matching within the image boundaries
...
def Step1_fast_match(_noisyImg, _BlockPoint):
    # finds the similar block fort block matching and applying hard thresholding
...
def Step1_3DFiltering(_similar_blocks):
    # applying hard thresholding to filter the grouped blocks
    statis_nonzero = 0  
...
def Aggregation_hardthreshold(_similar_blocks, blk_positions, m_basic_img, m_wight_img, _nonzero_num, Count, Kaiser):
    # Aggregation of the filter blocks to form the basic estimate
...
def BM3D_1st_step(_noisyImg):
    # first step of the BM3D algorithm
...
def Step2_fast_match(_Basic_img, _noisyImg, _BlockPoint):
    # Weiner filtering for the second step of the BM3D algorithm
    # matching for the block with the basic estimated image
    (present_x, present_y) = _BlockPoint 
...
def Step2_3DFiltering(_Similar_Bscs, _Similar_Imgs):
    # aopplying hard thresholding for the transformation 
...
def Aggregation_Wiener(_Similar_Blks, _Wiener_wight, blk_positions, m_basic_img, m_wight_img, Count, Kaiser):
    # Aggregation to form the basic estimate( denoised image )
...
def BM3D_2nd_step(_basicImg, _noisyImg):
    # Applying the second stage of the BM3D algorithm
    (width, height) = _noisyImg.shape
